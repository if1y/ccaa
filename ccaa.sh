...
if [ $1 = 'cdn' ]
	...
	master_url='https://github.com/if1y/ccaa/archive/master.zip'
...
function setting(){
	...
	#启动服务
	nohup sudo -u root aria2c --conf-path=/etc/ccaa/aria2.conf > /var/log/aria2.log 2>&1 &
	#nohup caddy -conf="/etc/ccaa/caddy.conf" > /etc/ccaa/caddy.log 2>&1 &
	nohup sudo -u root /usr/sbin/ccaa_web > /var/log/ccaa_web.log 2>&1 &
	#运行filebrowser
	nohup sudo -u root filebrowser -c /etc/ccaa/config.json > /var/log/fbrun.log 2>&1 &
...
#卸载
function uninstall(){
	wget -O ccaa-uninstall.sh https://raw.githubusercontent.com/if1y/ccaa/master/uninstall.sh
